...
def fit_box_to_step_function_science(y,initial_guess, x = None, ):
    ...
    trend = minimum_filter(median_filter(y, 5) , 3)
    y = y - trend
    ...
    params.add('center', value=initial_guess[1], min=60, max = 80)
    params.add('width', value=initial_guess[2], min=1e-3, max = 0.5*(np.max(x) - np.min(x)) )


    params.add('off', value=off, min=np.min(y), max = 0.4*np.max(y) )
